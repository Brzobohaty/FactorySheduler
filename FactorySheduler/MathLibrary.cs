...
namespace FactorySheduler
{
    ...
    public class MathLibrary
    {
        ...
        public static Point getPointOnLine(double x, double y, int angle, double distance)
        {
            double anglee = (angle * (Math.PI / 180));
            return new Point((x + (distance * Math.Cos(anglee))), (y + (distance * Math.Sin(anglee))));
        }
        ...
        public static Point[] getPointOnLineInDistance(Line line, Point origin, double distance)
        {
            ...
            {
                endPoint[0].Y = line.getY(roots.Item1.Real);
                endPoint[1].Y = line.getY(roots.Item2.Real);
            }
        ...
        /// <summary>
        /// Určí, zda je bod na levé straně vektoru
        /// </summary>
        /// <param name="startPoint">počáteční bod vektoru</param>
        /// <param name="endtPoint">koncový bod vektoru</param>
        /// <param name="point">bod, který nás zajímá</param>
        /// <returns>true pokud je na levé straně</returns>
        public static bool isPointOnTheLeftSideOfVector(Point startPoint, Point endtPoint, Point point)
        {
            return ((endtPoint.X - startPoint.X) * (point.Y - startPoint.Y) - (endtPoint.Y - startPoint.Y) * (point.X - startPoint.X)) > 0;
        }
